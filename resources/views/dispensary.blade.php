<x-app-layout>
    <x-slot name="header">
      ...
      @endif
      <p class="mt-1 text-slate-500 dark:text-slate-300 truncate mx-4">{{ $dispensary->official_license_type }}</p>
    </x-slot>
    <x-slot name="scripts">
    ...
    <div class="max-w-7xl mx-auto pb-12 px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div class="flow-root mt-6 bg-white dark:bg-slate-800 rounded-lg">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-slate-700 dark:text-slate-200">
                <div class="w-full text-lg flex items-center justify-between p-6 space-x-6">
                    <div class="max-w-2xl grid grid-cols-3 row-auto w-full gap-2 mt-2 tracking-wide">
                ...
                <div>
                  <div class="-mt-px flex divide-x divide-slate-200">
                    ...
                    <div class="w-0 flex-1 flex">
                      <a href="mailto:{{ $dispensary->email }}" class="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-4 text-slate-700 dark:text-slate-100 font-medium border border-transparent rounded-bl-lg hover:text-slate-500">
                    ...
                    <div class="-ml-px w-0 flex-1 flex">
                      <a href="tel:{{ $dispensary->phone_number }}" class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-slate-700 dark:text-slate-100 font-medium border border-transparent rounded-br-lg hover:text-slate-500">
                  ...
                  <div>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 m-4">
                      ...
                      <div class="relative rounded-lg border border-slate-300 dark:border-slate-500 bg-white dark:bg-slate-800 dark:hover:bg-slate-700 px-6 py-5 shadow-sm flex items-center space-x-3 hover:border-slate-400 dark:hover:border-slate-700 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                          <div class="flex-1 min-w-0">
                              <p class="text-lg font-medium text-slate-900 dark:text-slate-50 truncate">
                              {{ $recall->name }}
                              </p>
                              <p class="text-slate-500 dark:text-slate-300 truncate">
                                Involved in {{ $dispensary->recalls->count() }} recalls
                              </p>
                          </div>
                          <div class="text-right">
                              <a href="/recall/{{$recall->id}}" class="inline-flex items-center shadow-sm px-2.5 py-0.5 border border-slate-300 dark:border-slate-600 leading-5 font-medium rounded-full text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">View recall</a>
                          </div>
                      </div>
                      @endforeach
                  </div>
