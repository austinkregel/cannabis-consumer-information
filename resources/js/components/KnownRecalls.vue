<template>
    <div v-if="$store.getters.recalls?.length > 0" class="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-4 bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg py-4 px-4 mt-6">
        <template v-for="recall in $store.getters.recalls" :key="recall.id">
            <div class="flex flex-col justify-between text-slate-900 bg-slate-100 dark:bg-slate-900 dark:text-white py-2 px-4 rounded shadow">
                ...
                <div class="flex justify-center gap-12 my-4 text-2xl">
                    <div class="flex items-center justify-center gap-2 ">
                        <ShoppingCartIcon class="w-10 h-10 text-slate-100" />
                        <span v-text="recall.products_count"></span>
                    </div>
                    <div class="flex items-center justify-center gap-2">
                        <OfficeBuildingIcon class="w-10 h-10 text-slate-100" />
                        <span v-text="recall.dispensaries_count"></span>
                    </div>
                </div>
