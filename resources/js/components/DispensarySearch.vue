<template>
    <div>
        <div class="bg-white dark:bg-slate-700 rounded-lg shadow px-5 py-6 sm:px-6">
          <div>
            <label for="dispensary_name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Dispensary Name/License Number</label>
            <div class="mt-1 flex rounded-md shadow-sm">
              <div class="relative flex items-stretch flex-grow focus-within:z-10">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <QrcodeIcon class="h-5 w-5 text-slate-400 dark:text-slate-100" aria-hidden="true" />
                </div>
                <input v-model="code" type="dispensary_name" name="dispensary_name" id="dispensary_name" class="dark:text-slate-200 focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-none rounded-l-md pl-10 sm:text-sm border-slate-300 dark:border-slate-500 dark:bg-slate-600" placeholder="1A40530000..., 1A405..." />
              </div>
                ...
                <div v-for="recall in recalls" :key="recall.id">
                    <div class="rounded-md bg-yellow-50 dark:bg-yellow-700 p-4">
                        <div class="flex flex-col">
                            ...
                            <div class="ml-6">
                                <a target="_blanket" class="text-sm font-medium text-yellow-800 dark:text-yellow-50 underline" :href="recall.mra_public_notice_url">Learn More</a>
                            </div>
                        </div>
                    </div>
                </div>
