<template>
    <div>
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow px-5 py-6 sm:px-6">
          <div>
            <label for="metrc_code" class="block font-medium text-slate-700 dark:text-slate-300">Metrc Product Code</label>
            <div class="mt-1 flex rounded-md shadow-sm">
              <div class="relative flex items-stretch flex-grow focus-within:z-10">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <QrcodeIcon class="h-5 w-5 text-slate-400 dark:text-slate-100" aria-hidden="true" />
                </div>
                <input v-model="code" type="metrc_code" name="metrc_code" id="metrc_code" class="dark:text-slate-200 focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-none rounded-l-md pl-10 sm:border-slate-300 dark:border-slate-500 dark:bg-slate-700" placeholder="1A40530000..., 1A405..." />
              </div>
              <button @click="scan = !scan" type="button" class="-ml-px relative inline-flex items-center space-x-2 px-4 py-2 border border-slate-300 dark:border-slate-700 font-medium rounded-r-md text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-900 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                <VideoCameraIcon class="fill-current h-5 w-5 text-slate-400 dark:text-slate-200" aria-hidden="true" />
                <span>Scan</span>
              </button>
            </div>

              <button @click="searchForRecalls" type="button" class="-ml-px mt-2 relative inline-flex items-center space-x-2 px-4 py-2 border border-slate-300 dark:border-slate-700 font-medium rounded-md text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-900 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div v-if="decoded.length > 0" class="mt-5 md:col-span-2">
            ...
            <div class="shadow overflow-hidden sm:rounded-md">
                <div class="px-4 py-5 bg-white dark:bg-slate-800 sm:p-6 gap-6 flex flex-col divide-slate-200 dark:divide-slate-500 divide-y divide-solid -mt-4">
                    <div class="grid grid-cols-6 gap-4 pt-6" v-for="product in decoded" :key="product.id">
                        ...
                        <div class="col-span-6 sm:col-span-4">
                            <label class="block font-medium text-slate-700 dark:text-slate-300 mb-2">Dispensary Purchased At (Optional)</label>
                            <searchable-input
                            ...
                            >
                                <template #option="{option}">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-300">{{ option.license_type }} &mdash; {{ option.address }}</p>
                                    </div>
                                </template>
                            </searchable-input>
                        </div>
                        <div class="col-span-6 sm:col-span-4">
                            <label class="block font-medium text-slate-700 dark:text-slate-300 mb-2">Testing Facility (Optional)</label>
                            <searchable-input
                        ...
                        <div class="col-span-6 sm:col-span-4">
                            <label class="block font-medium text-slate-700 dark:text-slate-300 mb-2">Who grew it (Optional)</label>
                            <searchable-input
