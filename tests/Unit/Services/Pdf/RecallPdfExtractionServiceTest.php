...
class RecallPdfExtractionServiceTest extends TestCase
{
    ...
    public function testGetPackageIdsFromEncryptedPdfFile()
    {
        $filename = 'pdf.pdf';
        $pdfContent = "Pdf header \nSome more unrelated content\nPackage # 1A12345\n\tPackage#1A54321\n Package #1A98765\t\nPackage 1A87646";

        $this->pdfParserService->shouldReceive('getPdfTextFromFile')->once()->with($filename)->andThrow(new Exception('Secured pdf file are currently not supported.'));
